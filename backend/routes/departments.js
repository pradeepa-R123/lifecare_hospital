...
const DEPARTMENTS = [
  ...
  { name: "Pediatrics",       icon: "👶",  desc: "Comprehensive child & infant healthcare" },
  ...
  try {
    const depts = await Promise.all(
      DEPARTMENTS.map(async (d) => {
        const doctors = await User.find({ role: "Doctor", department: d.name }).select("name specialization experience");
        return { ...d, doctors };
      })
    );
    res.json(depts);
...
router.get("/:name", async (req, res) => {
  try {
    const dept = DEPARTMENTS.find(d => d.name.toLowerCase() === req.params.name.toLowerCase());
    if (!dept) return res.status(404).json({ message: "Department not found" });
    const doctors = await User.find({ role: "Doctor", department: dept.name }).select("-password");
